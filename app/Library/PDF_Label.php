...
class PDF_Label extends PDF_Code128
{
	...
	protected $_Avery_Labels = [
		...
		'L7651' => ['paper-size' => 'A4',		'metric' => 'mm',	'marginLeft' => 5,		'marginTop' => 11,		'NX' => 5,	'NY' => 13, 'SpaceX' => 2,		'SpaceY' => 0,		'width' => 38.1,	'height' => 21.1,	'font-size' => 9],
		'L7651_mod' => ['paper-size' => 'A4',	'metric' => 'mm',	'marginLeft' => 5,		'marginTop' => 11, 		'NX' => 5,	'NY' => 13,	'SpaceX' => 3.2,	'SpaceY' => 0.1,	'width' => 38.1,	'height' => 21.1,	'font-size' => 8,	'padding' => 1]
	];
	...
	function _Set_Format($format)
	{
		...
		if (isset($format['padding'])) {
			$this->_Padding = $this->_Convert_Metric($format['padding'], 'mm');
		} else {
	...
	function Add_BarLabel($text, $code = null)
	{
		$this->_COUNTX++;
		if ($this->_COUNTX == $this->_X_Number) {
			// Row full, we start a new one
		...
		$this->SetXY($_PosX, $_PosY);
		if ($code) {
			$this->Code128($_PosX, $_PosY, $code, 32, 4);
